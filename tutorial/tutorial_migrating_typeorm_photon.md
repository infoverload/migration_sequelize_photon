...
In the Sequelize project example, the data source and credentials can be defined in the [`ormconfig.json`](https://github.com/infoverload/migration_typeorm_photon/blob/typeorm/ormconfig.json) file:
...
In our sample Sequelize project:
...
`Task` and `User` are mapped to database tables. The fields are mapped to columns of the tables. Note that the `@id` directive indicates that this field is used as the _primary key_. 
- default
- updatedAt
- unique
